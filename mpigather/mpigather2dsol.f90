program mpigather2d
  ...
  integer :: vector, resizevector;
  integer, dimension(NPROC) :: recvcounts, displacements;
  ...
  call MPI_Type_vector(N, MP, M, MPI_INTEGER, vector, ierr)
  call MPI_Type_create_resized(vector, offset, newsize, resizevector, ierr)
  call MPI_Type_commit(resizevector, ierr)
  ...
  do i = 1, ROWP
      do j = 1, COLP
         displacements((i-1)*COLP+j) = (i-1)*MP + (j-1)*NP*M
      end do
   end do

  call MPI_Gatherv(localmat, MP*NP, MPI_INTEGER, &
	      matrix, recvcounts, displacements, resizevector, &
	      0, comm, ierr)

  if (rank == 0) then

     write(*,*) 'matrix on rank 0 after vector gather'
     write(*,*)
