...
subroutine haloswap_early_recv(x, m, n, comm)
  ...
  !  Receive lower boundaries and send upper ones
...
!  Receive upper boundaries and send lower ones
...
end subroutine haloswap_early_recv

subroutine haloswap_early_ssend(x, m, n, comm)
  ...
  if (rank /= size-1) then
     ...
     call mpi_ssend(x(1,n), m, MPI_DOUBLE_PRECISION, rank+1, &
                    tag, comm, ierr)
...
!  Send lower boundaries and receive upper ones
  ...
  if (rank /= 0) then
     ...
     call mpi_ssend(x(1,1), m, MPI_DOUBLE_PRECISION, rank-1, &
                    tag, comm, ierr)
...
end subroutine haloswap_early_ssend

subroutine haloswap_early_send(x, m, n, comm)
...
end subroutine haloswap_early_send
